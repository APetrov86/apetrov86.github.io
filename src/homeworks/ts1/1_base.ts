...
type StringArray<T = string | number> = {
  value: T;
  number: number;
};
 ...
 * Record - Record<Keys, Type> - создает тип, который является записью с ключами, определенными в первом параметре, и значениями типа, определенного во втором параметре.
...
export const transformCustomers = (customers: ICustomer[]): Record<number, Omit<ICustomer, 'id'>> => {
  return customers.reduce((acc, customer) => {
    acc[customer.id] = { name: customer.name, age: customer.age, isSubscribed: customer.isSubscribed };
    return acc;
  }, {} as Record<number, Omit<ICustomer, 'id'>>);
};
